# USA TODAY/AP/Northeastern University mass killing data project front-end
...
If you would like to copy data from the `dev` CDN folder (maybe you are working on something that requires a new piece of data or format that you are developing in the backend repo, [read more about the data here](#about-the-data)), you can load it with:

```
npm run data:development
```

You can also run the project locally and pull data directly from the `production` or `development` server location, without copying it, by running:
...
By default, this will build production-ready files, using production data and upload them to the production CDN folder.
